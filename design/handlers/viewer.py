...
class ViewerController:
    ...
    Entrada: app (MenuApp)
    Salida: None
    Descripción: Initialize the ViewerController with the parent app and default state.
    """
    ...
    Entrada: key (int)
    Salida: bool
    Descripción: Dispatch a key press to either the files list or the active view.
    """
    ...
    Entrada: key (int)
    Salida: bool
    Descripción: Handle navigation keys while in the file list view.
    """
    ...
    Entrada: key (int)
    Salida: bool
    Descripción: Handle navigation keys while in the packet or CSV view.
    """
    ...
    Entrada: None
    Salida: None
    Descripción: Load capture files from common data/output directories.
    """
        ...
        if not self._files:
            for d in ["outputs", "data/pcap", "data/metadata", "."]:
                ...
                if self._files:
                    break
        self._file_cursor = 0
        EVENT_LOG.info(f"Viewer: {len(self._files)} files found")
        self._refresh()

    """
    Entrada: None
    Salida: None
    Descripción: Open the currently selected file and parse it as PCAP or CSV.
    """
    def _open_file(self):
        ...
        if f["type"] in ("pcap", "pcapng"):
            ...
            if self._error:
                ...
                EVENT_LOG.ok(f"PCAP: {len(self._packets)} packets loaded")
        elif f["type"] == "csv":
            ...
            if self._error:
                ...
                EVENT_LOG.ok(f"CSV: {len(self._csv_rows)} rows, {len(self._csv_headers)} columns")
        self._refresh()

    """
    Entrada: None
    Salida: None
    Descripción: Rebuild the viewer section from the current state.
    """
    def _refresh(self):
