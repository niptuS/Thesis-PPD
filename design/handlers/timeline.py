...
class TimelineController:
    ...
    Entrada: None
    Salida: None
    Descripción: get target devices
    """
    ...
    Entrada: None
    Salida: str
    Descripción: Legacy: return first attacker IP.
    """
    ...
    Entrada: stdscr
    Salida: str | None
    Descripción: Smart attacker selection: auto if 1, selector if multiple.
    """
    def _select_attacker(self, stdscr) -> str | None:
        ...
        if not profs:
            EVENT_LOG.error("No attackers registered (role=attacker in Devices)")
            return None
        if len(profs) == 1:
        ...
        if ctrl_d:
            ...
            if attackers:
                return device_select_overlay(stdscr, "Source attacker", attackers)
        return profs[0].device_ip

    """
    Entrada: device_ip
    Salida: list
    Descripción: Get benign actions from the profile linked to this device IP.
    """
    ...
    def _do_add(self):
        ...
        etype = choice_select_overlay(stdscr, "Event type", EVENT_TYPES, "attack")
        if not etype:
            return

        targets = self._get_target_devices()
        if not targets:
            EVENT_LOG.error("No target devices. Scan first")
            return
        target_ip = device_select_overlay(stdscr, "Target device", targets)
        if not target_ip:
            return
        ...
        if etype == "attack":
            ...
            if not attacks:
                EVENT_LOG.error("No attacks in library")
                return
            ...
            if profile_actions:
                ...
                action = choice_select_overlay(stdscr, "Benign action", action_names, action_names[0])
                if not action:
                    return
            else:
        ...
        now = datetime.now()
        if base <= now:
            base_label = f"from now ({now.strftime('%H:%M:%S')})"
        else:
            base_label = f"from {base.strftime('%H:%M:%S')}"
        ...
        if etype == "attack":
            ...
            if attack_def and attack_def.continuous:
                ...
                dur_str = time_wheel_overlay(
                    stdscr,
                    f"Duration ({attack_def.tool}, recommended {rec}s)",
                    rec_hms,
                )
                if dur_str is None:
                    return
    ...
    def _do_edit(self):
        ...
        fields = ["action", "target", "source", "offset", "duration", "type", "notes", "Cancel"]
        field = choice_select_overlay(stdscr, "Edit field", fields, fields[0])
        if not field or field == "Cancel":
            return

        if field == "type":
            new = choice_select_overlay(stdscr, "Type", EVENT_TYPES, ev.event_type)
            if new:
                ev.event_type = new
        elif field == "action":
            if ev.event_type == "attack":
                ...
                new = text_input_overlay(stdscr, "Action", ev.action)
            if new:
                ev.action = new
                ev.label = new
        elif field == "source":
            ...
            targets = self._get_target_devices()
            if targets:
                new = device_select_overlay(stdscr, "Target", targets, ev.target)
                if new:
                    ev.target = new
        elif field == "offset":
            ...
            new = time_wheel_overlay(stdscr, "Duration", dur_hms)
            if new is not None:
                ev.duration_s = self._parse_time(new)
        elif field == "notes":
            new = text_input_overlay(stdscr, "Notes", ev.notes)
            if new is not None:
                ev.notes = new
        ...
        self._cursor = self._manager.find_index(ev.event_id)
        self._sync_page()
        EVENT_LOG.info(f"Event edited: {ev.label}")
        self._refresh()
    ...
    def _do_delete(self):
        ...
        self._clamp_cursor()
        EVENT_LOG.info(f"Event deleted: {ev.label}")
        self._refresh()
    ...
    def _do_move(self):
        ...
        if self._app._stdscr is None:
            return
        new_time = time_wheel_overlay(self._app._stdscr, "New offset", ev.offset_str())
        if new_time is None:
            return
        ...
        self._cursor = self._manager.find_index(ev.event_id)
        self._sync_page()
        EVENT_LOG.info(f"Event moved to {ev.offset_str()}: {ev.label}")
        self._refresh()

    """
    Entrada: val
    Salida: int
    Descripción: parse time
    """
    @staticmethod
